...
from src.dialog.win_mlog import Win_Mlog
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.pb_font.clicked.connect(self.change_font)
        self.cb_log_detail.stateChanged.connect(self.log_set)
        ...
        self.tbl_2.viewport().installEventFilter(self)
        # self.tbl_2 = MyTableWidget(self.tab)
        # self.tbl_2.setGeometry(410,130,635,271)
        # self.tbl_2.setRowCount(10)
        # self.tbl_2.setColumnCount(4)
        ...
        if(sys.platform in ['win32','win64']):
            self.setStyleSheet('font: 11pt "汉仪文黑-85w";')
            # self.groupBox5.setStyleSheet('font: 10pt "汉仪文黑-85W";')
            # self.groupBox10.setStyleSheet('font: 10pt "汉仪文黑-85W";')
            self.tbl_2.setStyleSheet('font: 10pt "汉仪文黑-85W";')

        if(sys.platform=='darwin'):
            self.setStyleSheet('font: 81 13pt "HYWenHei";')
            self.tbl_2.setStyleSheet('font: 81 12pt "HYWenHei";')
        font.setPointSize(9)
        ...
        self.pb_mlog.clicked.connect(self.win_mlog.display)
    ...
    def log_set(self):
        ...
        if self.cb_log_detail.isChecked():
            ...
            logging.getLogger('Buff').setLevel(logging.INFO)
        else:
    ...
    def eventFilter(self, source, event):
        if (event.type() == QtCore.QEvent.MouseButtonDblClick and
            ...
            source is self.tbl_2.viewport()):
            item = self.tbl_2.itemAt(event.pos())
            if item is not None:
                print('dblclick:', item.row(), item.column())
                if (item.row(),item.column()) in self.lookup:
                    print(self.lookup[item.row(),item.column()])
                    self.win_show_one = Rec_Artifact(self._aeffect,self.statusBar())
                    self.win_show_one.load_entry(self.lookup[item.row(),item.column()])
                    self.win_show_one.groupBox_2.hide()
                    self.win_show_one.pb_link_pic.hide()
                    self.win_show_one.pb_openimg.hide()
                    self.win_show_one.show()
        return super(MainWindow, self).eventFilter(source, event)

    def change_font(self):
        self.setStyleSheet(('font: 81 {}pt "HYWenHei";'.format(self.sb_font_main.value())))
        self.tbl_2.setStyleSheet(('font: 81 {}pt "HYWenHei";'.format(self.sb_font_sub.value())))
