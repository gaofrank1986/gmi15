...
class caraWindow(QtWidgets.QDialog):
    def __init__(self,alist,path,lookup=None):
        ...
        for row in range(1,num_row+1):
            for column in range(1,num_col+1):
                try:
                    ...
                    icon = ClickableLabel(50, 50,item,self.path)
                    ...
                    icon.cb.setFixedHeight(20)
                    hlay.addWidget(icon.cb)
                    hlay.addWidget(icon)
        ...
        # if not (lookup is None):
        #     self.setMinimumHeight(num_row*(80+44))
        # else:
        #     # self.setMinimumHeight(num_row*(80+5))
        #     pass

    def handleLabelClicked(self, name):
        print('"%s" clicked' % name)
        tmp = self.findChild(ClickableLabel,name)
        if tmp.cb.isChecked():
            tmp.cb.setChecked(False)
        else:
            tmp.cb.setChecked(True)
        # print(tmp)
        self.cara = name
        self.clicked = True
        # self.close()
